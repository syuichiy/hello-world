...
- 📈 **価格推移タブ（投資金額に対する評価額の比率）**：「価格推移」タブで、各保有の**評価額 ÷ 投資金額（%）**の推移をグラフと表で表示。価格の水準差があっても比べやすいよう、投資金額を100%に正規化して1つのグラフに重ねます（縦軸＝比率%、横軸＝日付、100%＝元本の基準線つき）。表は月末の比率を色分け表示。価格は**アプリ起動時（初回取得時）のスナップショット**を保持します。比率の計算には「銘柄一覧」で入力する**口数**と**投資金額**を使います
