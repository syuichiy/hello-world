...
- 📈 **価格推移タブ（取引履歴・実額ベース）**：「価格推移」タブで、取引履歴の**実額（評価額）**を元にした正確な推移を表示します。上部に〈投資金額合計／現在評価額合計／損益／損益率〉のサマリ、グラフは各商品を**投資金額＝100%**に正規化した**評価額÷投資金額（%）**の推移（太線＝ポートフォリオ全体、100%＝元本の基準線、凡例クリックで個別表示切替）、その下に**金額の推移（評価額・円）**を日付×商品の表で表示します。データは同梱の取引履歴（`portfolio_seed.json`）を初回起動時に取り込みます
