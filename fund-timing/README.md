...
- 🎲 **1銘柄への集中を「持ち続ける vs 売って分散する」で比較**：資産の15%以上を1銘柄が占めているとき、取り崩し戦略タブに比較表が出ます。**ブレなしの計算では持ち続けるほうが有利**（売れば譲渡益税を払うぶん資産が減るため）ですが、値動きのブレを入れると結果が変わりえます。集中銘柄の変動率・それ以外の変動率・両者の相関を実際の価格から測り、比率に応じた合成の変動率でモンテカルロを回して、〈100歳まで尽きない確率／100歳時点の中央値・下位10%〉を比べます。売却は「使う」のではなく**同額を分散資産に買い直す**前提で、NISAの残り枠を先に使います。両案は**同じ乱数列**で走らせるので、差は方針の違いだけによるものです
