...
- 🎲 **1銘柄への集中を「どれだけ残すか」で比較**：資産の15%以上を1銘柄が占めているとき、取り崩し戦略タブに〈全部売る／5%・10%・15%・20%残す／現状のまま持ち続ける〉の比較表が出ます。集中銘柄の変動率・それ以外の変動率・両者の相関を実際の価格から測り、残す比率に応じた合成の変動率 σ＝√(w²σ銘柄²＋(1−w)²σその他²＋2w(1−w)ρσ銘柄σその他) でモンテカルロを回して、〈全体の変動率／その水準まで下げるのにかかる年数／100歳まで尽きない確率／100歳時点の中央値・下位10%〉を並べます。**「少し残す」コストは小さく、「多く残す」コストは急に大きくなる**ことが数字で見えます。売却は「使う」のではなく**同額を分散資産に買い直す**前提で、NISAの残り枠はどの案でも先に使います。すべての案を**同じ乱数列**で走らせるので、差は方針の違いだけによるものです（勤務先の株の場合、給与・退職金も同じ会社に依存するぶん実質のリスクはこれより大きくなります）
