...
- ➕ **内蔵の商品をポートフォリオ画面から追加**：ポートフォリオ画面の「内蔵の商品を追加」で、内蔵の人気商品を資産クラスごとに一覧・絞り込みし、証券会社を選んでワンクリックで追加できます。バリュー系（農林中金おおぶね／スパークス厳選投資）、高配当系（楽天・SCHD／Tracers 日経平均高配当株50）、インド株、金（純金ファンド）など52本を内蔵
...
- 📤 **CSVはブラウザ側で読んでからJSONで送る**：ファイルをそのまま送る（multipart）方式だと、**iCloud Driveなどで中身がまだ手元に無いファイル**は送信が始まらず、サーバーにも届かないまま「読み取り中…」で止まります。先にブラウザでファイルを読み切ってからJSONで送るようにしたので、読めないときは**理由（iCloudのダウンロード未完了・アクセス不可）を画面に出します**（30秒で打ち切り）
- 📋 **CSVの中身を貼り付けても取り込める**：ファイル選択が使えない・アップロードが進まない端末のために、設定タブの取り込みカードに**貼り付け用の入力欄**を用意しました。CSVをテキストエディット等で開いて全選択→コピーし、貼り付けて「この内容を読み取る」を押すと、ファイル選択と同じ確認画面に進みます。この経路は**JSONで送る**ので、ファイル送信（multipart）が通らない環境でも取り込めます
- 🧷 **CSV取り込みは止まらず理由を出す**：読み取りは60秒・取り込みは120秒で区切り、サーバーがJSON以外を返したときは**HTTPステータスと中身の先頭**を画面に出します（「読み取り中…」のまま応答が返らない状態にならないようにするため）。CSVの解析で想定外のエラーが出てもサーバー側で受け止めて理由を返します。iPadのファイルAppでCSVが選べないことがあるため、ファイル選択の対象も広げています
...
- 🧭 **これからの投資アクション（AI総合）**：資産プランタブの先頭に、**各タブのAIコメントを横断した**助言カードを置きました。〈資産と口座の内訳・含み益と概算税／資産クラスの配分と理想との差／一番大きい保有の比率／分配金／年齢・退職・年金・企業年金・一時金・生活費／取り崩しの試算（方式ごとの資産寿命と成功確率）／手元に置きたい現金の目安と過不足／直近の売買シグナル〉を**まとめて**渡し、**〈今すぐ／今年中／退職まで／退職後〉の優先度つきで「何を・なぜ・どう進めるか」**を3〜5件返します。見落としやすい点と、**いまのままでよいこと**も出るので、何もかも変えなくて済みます。数字は画面に出ているものだけを渡し、前提を変えると古い助言は消えます（設定でAIをオンにしている場合のみ表示）
...
起動すると、他端末で開くURLが表示されます。**IPアドレスは起動のたびに変わることがある**ので、
**そのとき表示されたURL**を開いてください（例）：
...
Wi-Fiと有線の両方につながっている場合など、**IPの候補が複数あるときは全部表示**します。
**どのインターフェイスのアドレスか**も併記するので、`en0（Wi-Fi・有線）`のものを選んでください。
`utun（VPN）`・`vmnet（仮想マシン）`・`bridge`・`awdl（AirDrop）`のアドレスでは、同じWi-Fiの端末からはつながりません（**VPNに接続していると、これが既定のIPとして選ばれてしまう**ことがあります）。

> つながらないときは、次の順に確認してください。
> 1. **`run.command` ではなく `run-lan.command` で起動しているか**（`run.command` は自分のMacからのみ）
> 2. **ターミナルに表示されているIPと同じか**（前回のIPは使えません。DHCPで変わります）
> 3. **ポート番号も表示どおりか**（8765が使用中だと 8766 などになります）
> 4. スマホ・タブレットが**同じWi-Fi**に接続しているか（4G/5G回線ではつながりません）
> 5. macOSの**ファイアウォール**で受信を許可しているか（システム設定 → ネットワーク → ファイアウォール）
...
まず内蔵の人気投信（`seed_funds.py`。eMAXIS Slim・楽天・SBI・V・雪だるま・ニッセイ・たわら・iFreeNEXT・FANG+・Tracers・ひふみ・野村・アライアンス・バーンスタイン・セゾン、インド株、金（ゴールド）など52本）が名前検索の対象です。ここに無い投信は、コードを入れれば登録でき、以降は名前で検索できます。
...
### 新しいMacへの移行（バックアップ・復元）

保存先はホーム直下なので、**Macを買い替えると引き継がれません**。
設定画面の「💾 バックアップ・データ移行」で、商品・保有・評価額の履歴・売買記録・設定を
1つのJSONファイルに書き出し、新しいMacで読み込みます（価格キャッシュは含まれません）。

1. 旧Macで **「⬇️ バックアップを保存」** → JSONファイルがダウンロードされます
2. 新Macへファイルを渡す。**AirDropが最も確実**です（数秒で届き、クラウドを経由しません）
3. 新Macで **「⬆️ バックアップから復元」** → ファイルを選ぶ

> 「移行アシスタント」でホームフォルダごと移した場合は `~/.fund-timing/funds.db` も
> コピーされているので、そのまま起動すれば登録内容が残っています。

#### 復元で「Load failed」と出る場合

Safariは、**ファイルを読めなかったとき**も**サーバーに送れなかったとき**も同じ
"Load failed" としか出しません。本アプリは段階を分けて原因を表示するので、メッセージで判断できます。

- **「ファイルを読み込めませんでした」** → ファイル側の問題です。
  **iCloud Driveにあるファイルは、実体がまだ手元に無い（雲アイコンが付いている）と読めません。**
  Finderで雲アイコンをクリックしてダウンロードするか、**デスクトップにコピー**してからやり直してください。
- **「通信が途中で切れました」** → アプリ側の問題です。`run.command` のターミナルが
  動いたままか確認してください（閉じているとサーバーが止まります）。
- **「このアプリのバックアップファイルではないようです」** → 別のJSONを選んでいます。
  ファイルの先頭に `"app": "fund-timing"` が入っているものを選んでください。

どうしてもファイルを読み取れないときは、**「ファイルを読み取れないときは、バックアップの中身を貼り付ける」**
を開き、JSONをテキストエディットで開いて全選択（⌘A）→ コピー（⌘C）して貼り付けてください。
ファイルの読み取りを使わないので、iCloud上のファイルでも復元できます。
...
設定画面の「利用モデル」で **ローカルLLM** を選ぶと、Claude（クラウド）の代わりに
自分のMacで動かしているモデルを使います。**APIキー不要・費用ゼロで、保有状況が外部に出ません。**
その代わり精度は落ち、1回の生成に数十秒〜数分かかります。

### 1. Ollamaを入れてモデルを取得

```bash
brew install ollama          # または https://ollama.com からアプリ版を導入
ollama serve                 # 常駐させる（アプリ版なら自動）
ollama pull qwen3:14b        # モデルを取得（約9GB）
```

### 2. アプリ側の設定

設定画面 →「利用モデル」で **ローカルLLM** を選び、

| 欄 | 入れる値 |
|---|---|
| 接続先 | `http://127.0.0.1:11434/v1`（Ollama）／ `http://127.0.0.1:1234/v1`（LM Studio） |
| モデル名 | `ollama list` で出る名前（例 `qwen3:14b`） |

「保存」を押すと、その場でAIコメントの生成が始まります。
**「接続を確認」**を押すと、生成を待たずに、つながるか・そのモデルが入っているかが分かります。

### いまどのモデルで動いているか

- **AIコメントの右肩**に、実際に生成したモデル名が出ます（`gpt-oss:20b` など）
- 設定画面の**「接続を確認」**で、接続先・使われるモデル・サーバーに入っている一覧が出ます
- ターミナルからは `ollama ps`（読み込み中のモデル）／ `ollama list`（入っているモデル）で確認できます

### モデルを切り替える

```bash
ollama pull gpt-oss:20b      # 取得（1回だけ）
```

設定画面のモデル名の欄を `gpt-oss:20b` に書き換えて「保存」。
複数のモデルを入れておけるので、名前を書き換えるだけで行き来できます。

### メモリ別のモデルの目安

投信の助言は**指示に忠実に日本語のJSONを返せること**が要件で、知識量はあまり要りません
（数字はアプリが計算して渡しています）。量子化は4bit（Ollamaの既定）が前提です。

| 搭載メモリ | 目安のモデル | 必要な空きメモリ |
|---|---|---|
| 16GB | `qwen3:8b` / `gemma3:12b` | 5〜8GB |
| 24〜32GB | **`qwen3:14b`（おすすめ）** / `gpt-oss:20b` | 9〜13GB |
| 48GB以上 | `qwen3:32b` / `gpt-oss:120b` | 20GB〜 |

モデルのファイルサイズ＋2〜3GBが実際の使用量の目安です。
残りでmacOSとブラウザを動かす必要があるので、**搭載メモリの半分**を上限に選ぶと安定します。

### 全銘柄が同じコメント（多くは「売り」）になるとき

ローカルLLMの**コンテキスト長**（一度に読める量）を超えているのが原因です。
Ollamaの既定は4096トークンで、保有20本ぶんの指標を渡すと**超えたぶんが黙って捨てられ**、
モデルは銘柄ごとの数字を見ないままコメントを書くことになります。

本アプリは**小分けにして頼む**ことで対処しています
（全体コメント1回＋銘柄別を6件ずつ）。1回の入力は約1,900文字に収まるので、
既定のままでも切り捨てられません。それでも同じコメントが並ぶ場合は、
コンテキスト長を広げてください。
...
アプリ版のOllamaなら、設定画面の Context length を 8192 以上にします。
それでも改善しないときは、指示追従の強いモデル（`qwen3:14b` など）をお試しください。

### うまく動かないとき

- **「接続できませんでした」** → `ollama serve` が動いていません。ターミナルで実行してください
- **「モデルが見つかりません」** → `ollama pull <モデル名>` を実行するか、設定のモデル名を
  `ollama list` の表示に合わせてください
- **「応答を解釈できませんでした」** → そのモデルがJSONを返せていません。
  指示追従の強いモデル（`qwen3` 系）に変えると改善します
- **待たされる・途中で切れる** → モデルが大きすぎます。1つ小さいものをお試しください

> 構造化出力（json_schema）に対応していないサーバーでも動くよう、
> 対応していなければ自動でゆるい指定に切り替え、```json で囲まれた応答や
> 推論モデルの `<think>…</think>` が付いた応答も読めるようにしています。
