...
SEED_FUNDS = [
    ...
    {"name": "Tracers 日経平均高配当株50インデックス（奇数月分配型）",
     "isin": "JP90C000QF97", "assoc_code": "02313241", "category": "国内株式（高配当・分配型）"},

    # --- 全世界株式（オルカン以外の選択肢） ---
    {"name": "SBI・V・全世界株式インデックス・ファンド",
     "isin": "JP90C000MWH8", "assoc_code": "89311221", "category": "全世界株式"},
    {"name": "SBI・全世界株式インデックス・ファンド（雪だるま（全世界株式））",
     "isin": "JP90C000FQR3", "assoc_code": "8931217C", "category": "全世界株式"},
    {"name": "たわらノーロード 全世界株式",
     "isin": "JP90C000HZE8", "assoc_code": "47312197", "category": "全世界株式"},
    {"name": "eMAXIS Slim 全世界株式（3地域均等型）",
     "isin": "JP90C000G7W3", "assoc_code": "03312184", "category": "全世界株式"},

    # --- インド株（新興国の中でも人気） ---
    {"name": "iFreeNEXT インド株インデックス",
     "isin": "JP90C000PCX0", "assoc_code": "04314233", "category": "インド株式"},
    {"name": "SBI・iシェアーズ・インド株式インデックス・ファンド（サクっとインド株式）",
     "isin": "JP90C000PZX1", "assoc_code": "89311239", "category": "インド株式"},

    # --- 金（ゴールド） ---
    # 株式ではないので資産クラスは「グローバル」を仮置き。分散の役割が違うため、
    # 気になる場合はポートフォリオ画面の分類編集で変更できる。
    {"name": "SBI・iシェアーズ・ゴールドファンド（為替ヘッジなし）（サクっと純金）",
     "isin": "JP90C000PLM4", "assoc_code": "8931A236", "category": "金（ゴールド）"},
    {"name": "三菱UFJ 純金ファンド（ファインゴールド）",
     "isin": "JP90C0007G10", "assoc_code": "03311112", "category": "金（ゴールド）"},

    # --- その他の人気どころ ---
    {"name": "iFreeレバレッジ NASDAQ100",
     "isin": "JP90C000H0S5", "assoc_code": "0431218A",
     "category": "米国株式（NASDAQ・レバレッジ）"},
    {"name": "アライアンス・バーンスタイン・米国成長株投信Bコース（為替ヘッジなし）",
     "isin": "JP90C00025J4", "assoc_code": "39312065", "category": "米国株式（アクティブ）"},
    {"name": "セゾン・グローバルバランスファンド",
     "isin": "JP90C0004GZ7", "assoc_code": "96311073", "category": "バランス（株式＋債券）"},
]
...
SEED_ASSET_CLASS = {
    ...
    "02313241": "高配当",       # Tracers 日経平均高配当株50（奇数月分配）
    "89311221": "グローバル",   # SBI・V・全世界株式
    "8931217C": "グローバル",   # SBI・全世界株式（雪だるま）
    "47312197": "グローバル",   # たわらノーロード 全世界株式
    "03312184": "グローバル",   # Slim 全世界株式（3地域均等型）
    "04314233": "非米国",       # iFreeNEXT インド株
    "89311239": "非米国",       # サクっとインド株式
    "8931A236": "グローバル",   # サクっと純金（金は株式ではないので仮置き）
    "03311112": "グローバル",   # 三菱UFJ 純金ファンド（同上）
    "0431218A": "高リターン",   # iFreeレバレッジ NASDAQ100（2倍）
    "39312065": "米国",         # AB 米国成長株B（無分配・為替ヘッジなし）
    "96311073": "グローバル",   # セゾン・グローバルバランス（株式50＋債券50）
}
